...
def independent_pair(a1, a2):
    ...
    # if one action adds something and the other deletes it
    # thus ones add list is in other delete list or vice versa
    # also those list have to be non empty
    # i wrote this thing in 2 lines because its too big
    conflic_effect = bool((a1_add.issubset(a2_del) or a2_add.issubset(a1_del)) and not(a1_add == set()) and not(a2_add == set()))
    conflic_effect_2 = bool((a1_del.issubset(a2_add) or a2_del.issubset(a1_add)) and not(a1_del == set()) and not(a2_del == set()))

    # if one action cant come after another
    # if ones precond is in others delete
    conflict_other = bool(any(val in a1_pre for val in a2_del) or any(val in a2_pre for val in a1_del))

    return bool(not(conflic_effect_2 or conflic_effect or conflict_other))
