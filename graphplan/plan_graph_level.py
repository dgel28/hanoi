...
class PlanGraphLevel(object):
    ...
    def update_action_layer(self, previous_proposition_layer):
        ...
        for act in all_actions:
            act_pre = act.get_pre() #the preconditions
            if previous_proposition_layer.all_preconds_in_layer(act): # if all preconditions are already in the prev layer
                if all(not(previous_proposition_layer.is_mutex(p1, p2)) for p1 in act_pre for p2 in act_pre):
                    # cheks if none of the needed peconds are mutex with each other in previouse layer
    ...
    def update_mutex_actions(self, previous_layer_mutex_proposition):
        ...
        for act in current_layer_actions:
            for act2 in current_layer_actions:
                if mutex_actions(act, act2, previous_layer_mutex_proposition) and act != act2: # self explanitory
                    self.action_layer.add_mutex_actions(act, act2)
    ...
    def update_proposition_layer(self):
        ...
        current_layer_actions = self.action_layer.get_actions()
        "*** YOUR CODE HERE ***"
        props = dict() #{proposition: [act1, act2, act3,....]}
        dels = set() # not used, because of noOp
        for act in current_layer_actions:
            add = act.get_add() # the add list
            for ad in add: # ad is proposition in add list
                if ad not in props.keys(): # make a new key as proposition
                    props[ad] = set()
                props[ad].add(act) # so we know from wich actions it could have come from
        ...
        for prop in props.keys():
            if prop not in dels:
                new_prop = Proposition(prop.get_name()) # to not change the original prop, create new one
                new_prop.set_producers(list(props[prop])) # the actions from witch it came from
    ...
    def update_mutex_proposition(self):
        ...
        "*** YOUR CODE HERE ***"
        for prop in current_layer_propositions:
            for prop2 in current_layer_propositions:
                if mutex_propositions(prop, prop2, current_layer_mutex_actions) and prop != prop2:
                    self.proposition_layer.add_mutex_prop(prop, prop2)
...
def have_competing_needs(a1, a2, mutex_props):
    ...
    if a1 == a2:
        return False
    a1_pre = set(a1.get_pre()) # precondition list
    a2_pre = set(a2.get_pre())
    # if at least one pair of preconds are mutex, the the actions have competing needs
    return any(Pair(a,b) in mutex_props for a in a1_pre for b in a2_pre)
...
def mutex_propositions(prop1, prop2, mutex_actions_list):
    ...
    """
    "*** YOUR CODE HERE ***"
    acts_1 = prop1.get_producers() # actions thet the proposition might have originated from
    acts_2 = prop2.get_producers()

    # if at least one pair of actions from the producers of act1 and act2 are not mutex
    # then the pair of props can be achived directly without problems thus they are not mutex
    if any(Pair(i, j) not in mutex_actions_list and i!=j for i in acts_1 for j in acts_2):
        return False
    return True
